                        ...
                        <div class="card rounded-0 card-outline card-primary">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                @if ($user->can('department_slider.create'))
                                    <a href="{{ route('admin.academics.slider.create') }}" class="btn btn-sm btn-primary"><i
                                ...
                                <table
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Department</th>
                                            <th>Status</th>
                                            @if ($user->can('department_slider.edit') || $user->can('department_slider.delete'))
                                                <th>Action</th>
                                            @endif
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($slider as $item)
                                            <tr>
                                                ...
                                                @if ($user->can('department_slider.edit') || $user->can('department_slider.delete'))
                                                    <td>
                                                        @if ($user->can('department_slider.edit'))
                                                            <a title="Edit"
                                                                href="{{ route('admin.academics.slider.edit', $item->id) }}"
                                                                class="text-info mx-1"><i class="fas fa-edit"></i></a>
                                                        @endif
                                                        @if ($user->can('department_slider.delete'))
                                                            <a title="Delete"
                                                                onclick="return confirm('Are you sure to Delete?')"
                                                                href="{{ route('admin.academics.slider.destroy', $item->id) }}"
